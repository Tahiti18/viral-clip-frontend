...
export default function QueueWidget() {
  const [data, setData] = useState<QueueResp>(null);
  const [err, setErr] = useState<string | null>(null);
  useEffect(() => {
    async function run() {
      try {
        const r = await fetch(`${API_BASE}/v1/queue`, { cache: "no-store" });
  ...
  if (!API_BASE) return <div style={{color:'#b91c1c'}}>NEXT_PUBLIC_API_BASE is not set.</div>;
  if (err) return <div style={{color:'#b91c1c'}}>Queue error: {err}</div>;
  if (!data) return <div style={{opacity:0.7}}>Loading queue…</div>;
  return (
      ...
      <ul style={{listStyle:'none', padding:0, margin:0}}>
        {data.lanes?.map((l) => (
          <li key={l.id} style={{fontSize:14}}>
            ...
            {l.etaSeconds != null ? `• ETA ${Math.round(l.etaSeconds)}s` : ""}
          </li>
        ))}
      </ul>
    </div>
