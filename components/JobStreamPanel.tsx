...
export default function JobStreamPanel() {
  ...
  function start() {
    ...
    esRef.current?.close();
    const url = `${API_BASE}/v1/jobs/${encodeURIComponent(jobId)}/stream`;
    const es = new EventSource(url);
    es.onmessage = (e) => setLines((prev) => [...prev, e.data]);
    es.onerror = () => setLines((prev) => [...prev, "[stream error]"]);
    esRef.current = es;
  }
  return (
    <div style={{border:'1px solid #e5e7eb', borderRadius:8, padding:16}}>
      ...
      <pre style={{background:'#111827', color:'#bbf7d0', fontSize:12, padding:12, height:192, overflow:'auto', borderRadius:6, marginTop:12}}>
        {lines.join("\n")}
      </pre>
      {!API_BASE && <div style={{color:'#b91c1c'}}>Set NEXT_PUBLIC_API_BASE for streaming.</div>}
    </div>
  );
}
